...
class FootfallExcelController extends Controller
{
    // Xuất excel phần so sánh 2 cửa hàng
    public function sp_footfall_store_comparison_export_excel_v2(Request $request)
    {
        try {
            ...
            $organization_id_compare = $params->organization_id_compare;
            $site_id_compare = $params->site_id_compare;
            $start_time = '\'' . $params->start_time . '\'';
            $end_time = '\'' . $params->end_time . '\'';
            $start_date = $request->start_date;
            $end_date = $request->end_date;
            $view_by = $params->view_by;
            $indexOptionSelected = $params->indexOptionSelected;  // Visitors và Traffic;
            ...
            Excel::create('FOOTFALL_STORE_COMPARISON_' . $name, function ($excel) use ($visits, $traffic, $passer, $shopper, $turn_rate, $kids, $avg_time, $value_header, $index_not) {
                $header = array(
                    'dong1' => 'The results of counting the number of visitors in and out at the position : ' . $value_header['value1'] . ' ',
            ...
            $contentFromReport = "$org_name1 so sánh với $org_name2";
            $response = [
                'status' => 1, 'fileName' => $file_name, 'contentFromReport' => $contentFromReport
            ];
            return response()->json($response);
        } catch (\Exception $e) {
            $response = [
                'status' => 0, 'message' => $e->getMessage(), 'line' => $e->getLine()
            ];
            return response()->json($response);
        }
    }
    // Xuất excel cho visits, và metrics comparison
    public function sp_poc_data_in_out_sum_by_site_export_excel_v2(Request $request)
    {
        try {
            $request_user = $request->user();
            $user_id = $request_user != null ? $request_user->id : null;
            ...
            $operation = $params->operation;
            // // 'sum'
            $export = isset($params->export) ? $params->export : ''; // 'xuhuong', 'sum'
            $indexOption  = [];
            if (isset($params->traffic_index)) {
                $traffic_index =  $params->traffic_index;
            ...
            // Lấy tổng đánh giá để tính Percent
            $passer_by = 0;
            ...
            Excel::create('FOOTFALL_VISITS_' . $name, function ($excel) use ($export, $items1, $items2, $value_header) {
                $header = array(
                    'dong1' => 'The results of counting the number of visitors in and out at the position : ' . $value_header['value1'] . ' ',
                    'dong2' => 'The results of counting the number of visitors in and out at the position : ' . $value_header['value1'] . ' ',
            ...
            $response = [
                'status' => 1, 'fileName' => $file_name, 'contentFromReport' => $org_name
            ];
            return response()->json($response);
        } catch (\Exception $e) {
            $response = [
     ...
     public function sp_report_poc_raw_data_by_day_export_excel_v2(Request $request)
     {
         try {
    ...
    public function sp_report_poc_raw_data_by_month_export_excel_v2(Request $request)
    {
        try {
            // $request_user = $request->user();
            // $user_id = $request_user != null ? $request_user->id : null;
    ...
    public function sp_report_poc_raw_data_by_year_export_excel_v2(Request $request)
    {
        try {
            // $request_user = $request->user();
            // $user_id = $request_user != null ? $request_user->id : null;
    ...
    public function sp_footfall_store_comparison_export_excel(Request $request)
    {
        try {
            ...
            $indexOptionSelected = $request->indexOptionSelected;  // Visitors và Traffic;
    ...
    public function sp_footfall_time_comparison_export_excel(Request $request)
    {
        try {
            ...
            if ($row1 >= $row2) {
                foreach ($items as $key1 => $item) {
                    $visits[] = array(
                        'Time'       =>  $item->time_period . ((isset($itemsComapare[$key1]) && $view_by != "Hour") ? (' / ' . $itemsComapare[$key1]->time_period) : ''),
            ...
            if ($row2 > $row1) {
                foreach ($itemsComapare as $key1 => $item) {
                    $visits[] = array(
                        'Time'       =>  $item->time_period . (isset($items[$key1]) ? (' / ' . $items[$key1]->time_period) : ''),
            ...
            Excel::create('FOOTFALL_TIME_COMPARISON_' . $name, function ($excel) use ($visits, $traffic, $passer, $shopper, $turn_rate, $kids, $avg_time, $value_header, $index_not) {
                $header = array(
                    'dong1' => 'The results of counting the number of visitors in and out at the position : ' . $value_header['value1'] . ' ',
                    'dong2' => 'Time:  ' . $value_header['value2'] . '  ',
                    'dong3' => 'Date:  (' . $value_header['value3'] . ')  compared to  (' . $value_header['value4'] . ')'
                );
                // Tiêu đề ngoài file
                $excel->setCreator('ACS')->setCompany('ACS Solution');
    ...
    public function getSiteArray($siteArray, $selectCategory)
    {
        $returnData = [];
        $select = strtolower($selectCategory);
        foreach ($siteArray as $value) {
            $siteInfo = DB::table('sites')->where('id', $value)->where('actived', 1)->where('deleted', 0)->first();
            if ($siteInfo == null) {
                continue;
            }
            if ($select == 'group' && $siteInfo->store == 0) {
                $returnData[] = $siteInfo->id;
            } else if ($select == 'store' && $siteInfo->store == 1) {
                $returnData[] = $siteInfo->id;
            }
        }
        return $returnData;
    ...
    public function sp_footfall_heatmap_treemap_coloraxis_sum_reporting_export_excel_v2(Request $request)
    {
        try {
            $request_user = $request->user();
            $params = json_decode($request->params);
            ...
            $site_array = $this->getSiteArray($params->site_array, $select_category);
            $start_time = '\'' . $params->start_time . '\'';
            $end_time = '\'' . $params->end_time . '\'';
            $start_date = $request->start_date;
            $end_date = $request->end_date;
    ...
    public function get_value_header_store($start_date, $end_date, $start_time, $end_time, $org_name1, $org_name2)
    {
        ...
        $value_header = array(
            'value1' => $org_name1 .  ' compared to ' . $org_name2,
            'value2' => str_replace('\'', '', $start_time) . ' - ' . str_replace('\'', '', $end_time),
            'value3' => $ngay
        );
        return $value_header;
