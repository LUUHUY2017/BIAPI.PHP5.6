...
class PerformanceExcelController extends Controller
{
    ...
    public function sp_footfall_performance_boston_reporting_export_excel_v2(Request $request)
    {
        try {
    ...
    // Xuất excel heatmap
    public function sp_footfall_heatmap_treemap_coloraxis_sum_export_excel_v2(Request $request)
    {
        try {
            $request_user = $request->user();
            $user_id = $request_user != null ?  $request_user->id : null;
    ...
    public function sp_footfall_performance_data_by_site_export_excel_v2(Request $request)
    {
        try {
            $request_user = $request->user();
            $user_id = $request_user != null ?  $request_user->id : null;
    ...
    public function sp_footfall_performance_store_comparison_export_excel_v2(Request $request)
    {
        try {
            $request_user = $request->user();
            $user_id = $request_user != null ?  $request_user->id : null;
            ...
            $end_date = $request->end_date;
            $view_by = $params->view_by;
            $indexOptionSelected = $params->indexOptionSelected;  // Visits và Traffic;
            $operation = $params->operation;
            ...
            $contentFromReport = "$org_name1 so với $org_name2";
            $response = [
                'status' => 1
                , 'fileName' => $file_name
                , 'contentFromReport' => $contentFromReport
            ];
            return response()->json($response);
        } catch (\Exception $e) {
            $response = [
